...
def load_data(data_cfg: dict, multi_encoder: bool = False) -> (Dataset, Dataset, Optional[Dataset],
    ...
    prev_prev_prev_src_field = data.Field(init_token=CONTEXT_TOKEN, eos_token=CONTEXT_EOS_TOKEN,
                           pad_token=PAD_TOKEN, tokenize=tok_fun,
                           batch_first=True, lower=lowercase,
                           unk_token=UNK_TOKEN,
                           include_lengths=True)

    prev_prev_prev_trg_field = data.Field(init_token=CONTEXT_TOKEN, eos_token=CONTEXT_EOS_TOKEN,
                           pad_token=PAD_TOKEN, tokenize=tok_fun,
                           unk_token=UNK_TOKEN,
                           batch_first=True, lower=lowercase,
                           include_lengths=True)
    ...
    if multi_encoder:
        train_data = ContextTranslationDataset(path=train_path,
                                               ...
                                               fields=(prev_prev_prev_src_field, prev_prev_prev_trg_field, prev_prev_src_field, prev_prev_trg_field, prev_src_field, prev_trg_field, src_field, trg_field),
                                               filter_pred=
    ...
    #                             dataset=train_data, vocab_file=trg_vocab_file)
    #
    #     prev_prev_src_vocab = build_vocab(field="src_prev_prev", min_freq=src_min_freq,
    #                                     max_size=src_max_size,
    #                                     dataset=train_data, vocab_file=src_vocab_file)
    #
    #     prev_prev_trg_vocab = build_vocab(field="trg_prev_prev", min_freq=trg_min_freq,
    #                             max_size=trg_max_size,
    #                             dataset=train_data, vocab_file=trg_vocab_file)
    ...
    prev_trg_field.vocab = trg_vocab
    prev_prev_src_field.vocab = src_vocab
    prev_prev_trg_field.vocab = trg_vocab

    prev_prev_prev_src_field.vocab = src_vocab
    prev_prev_prev_trg_field.vocab = trg_vocab
...
class ContextTranslationDataset(Dataset):
    ...
    def __init__(self, path: str, exts, fields, **kwargs) -> None:
        ...
        if not isinstance(fields[0], (tuple, list)):
            fields = [('src_prev_prev_prev', fields[0]), ('trg_prev_prev_prev', fields[1]), ('src_prev_prev', fields[2]), ('trg_prev_prev', fields[3]), ('src_prev', fields[4]), ('trg_prev', fields[5]), ('src', fields[6]), ('trg', fields[7])]
        # fields = [('src', field)]
        ...
        with io.open(src_path, mode='r', encoding='utf-8') as src_file, \
            ...
            prev_src_line, prev_trg_line, prev_prev_src_line, prev_prev_trg_line, prev_prev_prev_src_line, prev_prev_prev_trg_line = CONTEXT_TOKEN + CONTEXT_EOS_TOKEN, CONTEXT_TOKEN + CONTEXT_EOS_TOKEN, CONTEXT_TOKEN + CONTEXT_EOS_TOKEN, CONTEXT_TOKEN + CONTEXT_EOS_TOKEN, CONTEXT_TOKEN + CONTEXT_EOS_TOKEN, CONTEXT_TOKEN + CONTEXT_EOS_TOKEN
            for src_line, trg_line in zip(src_file, trg_file):
                ...
                if src_line != '' and trg_line != '':
                    if src_line == 'removemeimaboundary' or trg_line == 'removemeimaboundary':
                        ...
                        continue
                    examples.append(data.Example.fromlist(
                        [prev_prev_prev_src_line, prev_prev_prev_trg_line, prev_prev_src_line, prev_prev_trg_line, prev_src_line, prev_trg_line, src_line, trg_line], fields))
                    prev_prev_prev_src_line = prev_prev_src_line
                    prev_prev_prev_trg_line = prev_prev_trg_line
